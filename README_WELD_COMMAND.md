...
- **功能**:
  ...
  - 连接到机器人控制器并发送该数据包。
  - **长连接**: 建立 TCP 连接后保持长连接，直到节点退出或发生网络错误。
...
### 长连接设计
当前版本已取消心跳机制，依赖 TCP 协议自身的 Keep-Alive 和 KAREL 空闲超时来保持连接稳定：

- **连接维护**: KAREL 与 ROS 节点依赖 TCP Keep-Alive 与空闲超时，无需额外心跳包。




- **优势**: 长连接降低网络频繁断开的风险，并简化了双方实现。
  - 保持长期稳定连接，避免因长时间无业务数据而误断开
  - 无需额外应用层报文，不影响焊接参数
  - 自动检测和恢复连接异常
- **ROS端优雅关闭**: 当你按 Ctrl+C 或 roslaunch 终止节点时，C++ 代码会先调用 `shutdown()` + `close()` 主动释放 TCP 连接，KAREL 端随即检测到断开并安全退出其内部循环，避免 "参数还没有设定" 报错及程序暂停

### 空闲超时保护
- **超时阈值**: 10秒无任何数据（业务指令）
- **触发条件**: 当 ROS 节点异常退出（如 Ctrl+C）或网络断开时，将触发空闲超时检测
...
5. **连接频繁断开**
   ...
   - **可能原因**: 网络抖动或 ROS 节点异常退出导致 TCP 断开。
   - **解决方案**: 
     - 确保使用最新版本的代码，并正确处理 socket 关闭逻辑
     - 检查 ROS 日志中是否有网络错误或多次重连的警告
     - 如果问题持续，可在 KAREL 日志中查看 "Disconnecting..." 或相关错误信息
...
2. **长连接验证**: 连接建立后保持静置 10 分钟以上，确认连接未意外断开。
